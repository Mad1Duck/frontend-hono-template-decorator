const GROUPS = [
  ...
  },
  {
    label: 'real-time',
    items: [
      { name: '@Sse(path?)',       desc: 'Registers a GET endpoint for SSE. Handler receives (c: Context, stream: SSEStreamingApi) — no decorator needed for the stream.' },
      { name: '@WebSocket(path?)', desc: 'Registers a WebSocket upgrade endpoint. Handler receives (c: Context) and returns { onOpen, onMessage, onClose }.' },
  ...
  },
  {
    ...
    items: [
      { name: 'Body(c, schema?)',            desc: 'Reads + validates request body. Pass a Zod schema for validation — returns 400 on failure. await Body(c, MySchema).' },
      { name: 'Query(c, schema?)',           desc: 'Reads query params. Pass a Zod schema to validate and type the result. await Query(c, FilterSchema).' },
      { name: 'Param(c, name)',              desc: 'Reads a route path parameter by name. Param(c, "id").' },
      { name: 'Headers(c, name?)',           desc: 'Reads a request header. Omit name to get all headers as an object.' },
      { name: 'User<T>(c)',                  desc: 'Returns the authenticated user object set by the guard. Typed: User<MyUser>(c).' },
      { name: 'Ip(c)',                       desc: 'Returns the real client IP (CF-Connecting-IP → X-Real-IP → X-Forwarded-For).' },
      { name: 'Device(c)',                   desc: "Returns detected device type: 'mobile' | 'tablet' | 'desktop' | 'bot'." },
      { name: 'UserAgent(c)',                desc: 'Returns the raw User-Agent header string.' },
      { name: 'Cookie(c, name)',             desc: 'Returns a single cookie value by name.' },
      { name: 'Cookies(c)',                  desc: 'Returns all cookies as Record<string, string>.' },
      { name: 'Req(c)',                      desc: 'Returns the Hono HonoRequest object.' },
      { name: 'Ctx(c) / Res(c)',             desc: 'Returns the full Hono Context c — for redirect, set-cookie, etc.' },
      { name: 'ValidatedBody(c, schema)',    desc: 'Alias for Body(c, schema) — emphasises validation intent.' },
      { name: 'ValidatedQuery(c, schema)',   desc: 'Alias for Query(c, schema).' },
      { name: 'ValidatedParam(c, name, schema)', desc: 'Reads a route param and validates it through a Zod schema.' },
      { name: 'UploadedFile(c, field)',      desc: 'Returns a single File from multipart form data by field name. Returns File | null.' },
      { name: 'UploadedFiles(c, field?)',    desc: 'Returns all File objects for a field. Omit field to collect every file in the form.' },
      { name: 'FormBody(c)',                 desc: 'Returns the raw FormData object. await FormBody(c).' },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { name: '@Injectable(tokens?)',           desc: 'Registers a class in the DI container. Pass explicit dependency tokens: @Injectable([Database, Logger]).' },
      ...
      { name: '@Stateless()',                   desc: 'Enforces that a @Singleton holds no mutable per-request state. Writes to the instance throw at runtime.' },
